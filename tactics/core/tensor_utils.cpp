...
Tensor::InsideDescribe::NativeInsideDescribe *
TensorUtils::get_describe(const Tensor *tensor) {
  return tensor->m_describe->m_content.get();
}
bool TensorUtils::region_is_full(Tensor *input) {
  auto des = TensorUtils::get_describe(input);
  int size = 1;
...
Tensor::InsideDescribe::Region TensorUtils::make_full_slice(Tensor *input) {
...
bool TensorUtils::reshape_slice(Tensor::InsideDescribe::Region &slice,
                                int outside, int inside, int axis) {
  if (slice.size[1] == 1 && slice.size[0] == 1 &&
      slice.size[2] == outside * inside * axis) {
  ...
  if (slice.size[0] == outside && slice.size[1] == axis &&
      slice.size[2] == inside) {
    return true;
  }
  return false;
...
void TensorUtils::setup_tensor_info(const Tensor *tensor, Tensor *wrapTensor,
                                    DATA_FORMAT mMidFormat) {
  TensorUtils::get_describe(wrapTensor)->dimension_format = mMidFormat;
  auto tensorFormat = TensorUtils::get_describe(tensor)->dimension_format;
  bool originCaffeFormat =
      (tensorFormat == DATA_FORMAT_NCHW || tensorFormat == DATA_FORMAT_NC4HW4);
  bool wrapCaffeFormat =
      (mMidFormat == DATA_FORMAT_NCHW || mMidFormat == DATA_FORMAT_NC4HW4);
  bool originTfFormat =
      (tensorFormat == DATA_FORMAT_NHWC || tensorFormat == DATA_FORMAT_NHWC4);
  bool wrapTfFormat =
      (mMidFormat == DATA_FORMAT_NHWC || mMidFormat == DATA_FORMAT_NHWC4);
  if ((originCaffeFormat && wrapCaffeFormat) ||
      (originTfFormat && wrapTfFormat)) {
    TensorUtils::copy_shape(tensor, wrapTensor);
  ...
  TensorUtils::set_linear_layout(wrapTensor);
  wrapTensor->buffer().type = tensor->getType();
}

void TensorUtils::copy_shape(const Tensor *source, Tensor *dest,
                             bool copyFormat, bool copyRef) {
  auto &ob = dest->buffer();
  auto &ib = source->buffer();
  ob.dimensions = ib.dimensions;
  ::memcpy(ob.dim, ib.dim, ib.dimensions * sizeof(halide_dimension_t));
  if (copyFormat) {
    get_describe(dest)->dimension_format =
        get_describe(source)->dimension_format;
  }
  if (copyRef) {
    auto dstDes = get_describe(dest);
    auto srcDes = get_describe(source);
    dstDes->regions = srcDes->regions;
    dstDes->quantAttr = srcDes->quantAttr;
    dstDes->type = srcDes->type;
    dest->buffer().type = source->getType();
  }
  adjust_tensor_for_compability(dest);
}

void TensorUtils::set_shape(Tensor *dest, const std::vector<int> &alldims) {
  auto &ob = dest->buffer();
...
void TensorUtils::set_linear_layout(Tensor *tensor) {
  ...
  for (int i = 0; i < buffer.dimensions; ++i) {
    ...
    if (1 == index &&
        tensor->m_describe->m_content->dimension_format == DATA_FORMAT_NC4HW4) {
...
static const Tensor *createHostPlanar(const Tensor *source) {
  // check
  auto bnType = FORWARD_CPU;
  auto tensorBackend = TensorUtils::get_describe_origin(source)->getBackend();
  if (tensorBackend) {
    bnType = tensorBackend->type();
  }
  bool device = bnType != FORWARD_CPU;
  bool chunky =
      TensorUtils::get_describe(source)->dimension_format == DATA_FORMAT_NC4HW4;
  ...
  // convert
  if (chunky) {
    Tensor *result = source->create_host_tensor_from_device(source, false);
    // if (result->getDimensionType() == Tensor::TENSORFLOW) {
    //     TensorUtils::get_describe(result)->dimension_format =
    //     DATA_FORMAT_NHWC;
    // } else {
    //     TensorUtils::get_describe(result)->dimension_format =
    //     DATA_FORMAT_NCHW;
    ...
    if (device) {
      void *host = ((Tensor *)source)->map(Tensor::MAP_TENSOR_READ);
      if (host != nullptr) {
        ::memcpy(result->buffer().host, host, result->size());
      }
      ((Tensor *)source)->unmap(Tensor::MAP_TENSOR_READ, host);
    } else {
      Backend::Info info;
      info.type = FORWARD_CPU;
      std::shared_ptr<Runtime> runtime(
          get_extra_runtime_creator(FORWARD_CPU)->on_create(info));
      auto backend = runtime->on_create();
      backend->on_copy_buffer(source, result);
      delete backend;
    }
    return result;
  } else {
    return source->create_host_tensor_from_device(source, true);
  }
}
...
static bool equals(const double *pa, const double *pb, size_t size,
                   double tolerance, double epsilon, bool overall,
                   bool prints) {
...
bool TensorUtils::compare_tensors(const Tensor *compare, const Tensor *expect,
                                  float tolerance, bool overall,
                                  bool printsErrors, bool printsTensors) {
  // type
  if (compare->getType().code != expect->getType().code ||
      compare->getType().bits != expect->getType().bits) {
    if (printsErrors) {
      printf("NOT equal in type: %d/%d - %d/%d.\n", compare->getType().code,
             compare->getType().bits, expect->getType().code,
             expect->getType().bits);
    }
    return false;
  }
  ...
  if (compare->dimensions() != expect->dimensions()) {
    if (printsErrors) {
      printf("NOT equal in dimensions: %d - %d.\n", compare->dimensions(),
             expect->dimensions());
    }
    return false;
  }
  for (int i = 0; i < compare->dimensions(); i++) {
    ...
    if (printsErrors) {
      printf("NOT equal in dimensions[%d]: %d - %d.\n", i, compare->length(i),
             expect->length(i));
    }
    return false;
  }
  ...
  if ((NULL != compareValue.data()) && (NULL != expectValue.data())) {
    result = equals(compareValue.data(), expectValue.data(), size, tolerance,
                    epsilon, overall, printsErrors);
  }
...
// is copy only region
bool TensorUtils::is_copy_region(const Tensor::InsideDescribe::Region &region) {
  bool eq = true;
  for (int i = 0; i < 3; i++) {
    eq &= ((region.src.stride[i] == region.dst.stride[i]) ||
           (region.size[i] <= 1));
  }
  return eq;
}

bool TensorUtils::is_transpose_region(
...
bool TensorUtils::is_tile_region(const Tensor::InsideDescribe::Region &region) {
  bool res = true;
  for (int i = 0; i < 3; i++) {
...
bool TensorUtils::is_depth_to_space_regions(const Tensor *output) {
  const auto &regions = TensorUtils::get_describe(output)->regions;
  if (regions.empty()) {
    return false;
  }
...
static inline int offsetCompute(const Tensor::InsideDescribe::Region &reg,
                                int srcOffset, int dstOffset, bool backward) {
  ...
  for (int i = 0; i < 3; i++) {
    if (reg.size[i] > 1) {
      res += (srcOffset / src->stride[i] - dstOffset / src->stride[i]) *
             dst->stride[i];
...
static inline bool expandSrc(std::vector<int> &src, std::vector<int> &dst,
                             std::vector<int> &size, int expandValue) {
  if (expandValue <= 0) {
    return false;
  }
...
static inline bool expandStrideSize(int *src, int *dst, int *size, int &num,
                                    int expandValue) {
#define MNN_3_INT_INSERT(x, i, y)                                              \
  if (i == 2) {                                                                \
    x[2] = y;                                                                  \
  } else if (i == 1) {                                                         \
    x[2] = x[1];                                                               \
...
bool TensorUtils::ref_tensor_content(Tensor *dst, const Tensor *src) {
  auto des = TensorUtils::get_describe(dst);
  auto srcDes = TensorUtils::get_describe(src);
  auto desO = TensorUtils::get_describe_origin(dst);
  auto srcDesO = TensorUtils::get_describe_origin(src);
  bool needMalloc = dst->buffer().host != src->buffer().host ||
...
static bool _ClipDst(int *stride, int srcOffset, int dstOffset,
                     const int *srcSize, const int *dstSize, const int sizeNum,
                     int *dstMax, int *dstMin) {
  /* Compute The range of dx, dy, dz:
   s0 * (dx-sx) + s1 * (dy-sy) + s2 * (dz-sz) + (doff-soff) = 0
   Assume the region won't be overlapped, then extract doff -> s0*xd+
   s1*yd+s2*zd, soff -> s0*xs+s1*ys+s2*zs xd-xs=xo, yd-ys=yo, zd-zs=zo then:
   dx-sx+xo = 0
   dy-sy+yo = 0
   dz-sz+zo = 0
...
static bool _RegionValid(int *stride, int offset, int *size, int sizeNum,
                         size_t limitSize) {
...
public:
  enum Status { FUSE_SRC_COPY, FUSE_DST_COPY, FUSE_REGION_COMPUTE };
  void apply(const Tensor::InsideDescribe::Region &srcReg,
             Tensor::InsideDescribe::Region &dstReg) {
    switch (mStatus) {
    case FUSE_SRC_COPY:
  ...
  bool match(const Tensor::InsideDescribe::Region &srcReg,
             const Tensor::InsideDescribe::Region &dstReg) {
    // dont deal size > 1 && stride <= 0
    for (int i = 0; i < 3; i++) {
      if (srcReg.size[i] > 1 &&
          (srcReg.src.stride[i] <= 0 || srcReg.dst.stride[i] <= 0)) {
        return false;
      }
      if (dstReg.size[i] > 1 &&
          (dstReg.src.stride[i] <= 0 || dstReg.dst.stride[i] <= 0)) {
        return false;
      }
    }
    bool copyValid = true;
    ...
    // Valid range is from srcReg: srcDstMin - srcDstMax, if dst's srcReg
    // exceed, not valid for copy
    if (srcDstMin > dstSrcMin || srcDstMax < dstSrcMax) {
      copyValid = false;
    }
    // src copy fuse
    if (is_copy_region(srcReg) && copyValid) {
      mStatus = FUSE_SRC_COPY;
      return true;
    }
    // dst copy fuse
    if (is_copy_region(dstReg) && dstTotalSize == srcTotalSize && copyValid) {
      mSrcOff = dstReg.src.offset - srcReg.dst.offset;
      mDstOff = dstReg.dst.offset;
      mSrcOff =
          offsetCompute(srcReg, dstReg.src.offset, srcReg.dst.offset, true) +
          srcReg.src.offset;
...
#undef MNN_3_INT_INIT
    ...
    sizeNum = dstNum;
#define MNN_3_INT_DIFF(r, x, y, i)                                             \
  if ((x[i] != y[0]) && (x[i] != y[1]) && (x[i] != y[2])) {                    \
    if (r > 0) {                                                               \
      return false;                                                            \
    } else {                                                                   \
      r = x[i];                                                                \
...
#undef MNN_3_INT_DIFF
    ...
    newSrcOffset =
        offsetCompute(srcReg, dstReg.src.offset, srcReg.dst.offset, true) +
        srcReg.src.offset;
    bool valid = _ClipDst(dstSrc, srcReg.dst.offset, dstReg.src.offset,
                          newSrcSize, dstSize, dstNum, dstMax, dstMin);
    if (!valid) {
      return false;
    }
...
private:
  ...
  int mDstOff;
  // general fuse
  int srcDst[3], srcSrc[3], dstSrc[3], dstDst[3], srcSize[3], dstSize[3],
      newSrc[3], dstStride[3], srcStride[3];
...
TensorUtils::FuseWrap::FuseWrap() { mStatus = new FuseRegionStatus; }
TensorUtils::FuseWrap::~FuseWrap() { delete mStatus; }
bool TensorUtils::FuseWrap::match(
    const Tensor::InsideDescribe::Region &srcReg,
    const Tensor::InsideDescribe::Region &dstReg) {
  return mStatus->match(srcReg, dstReg);
}
void TensorUtils::FuseWrap::apply(const Tensor::InsideDescribe::Region &srcReg,
                                  Tensor::InsideDescribe::Region &dstReg) {
  mStatus->apply(srcReg, dstReg);
}

void TensorUtils::adjust_tensor_for_compability(Tensor *newTensor) {
...
std::vector<float> TensorUtils::get_quant_info(const Tensor *t) {
  float scale =
      get_describe(t)->quantAttr ? get_describe(t)->quantAttr->scale : 0.0f;
  float zero =
      get_describe(t)->quantAttr ? get_describe(t)->quantAttr->zero : 0.0f;
  float min =
      get_describe(t)->quantAttr ? get_describe(t)->quantAttr->min : -127.0f;
  float max =
      get_describe(t)->quantAttr ? get_describe(t)->quantAttr->max : 127.0f;
  return {scale, zero, min, max};
}

Tensor::InsideDescribe *TensorUtils::get_describe_origin(const Tensor *tensor) {
  return tensor->m_describe;
}
size_t TensorUtils::get_raw_size(const Tensor *t) {
  size_t len = 1;
...
//     auto& regions = TensorUtils::get_describe(cmd->outputs[0])->regions;
...
int TensorUtils::get_tensor_channel_pack(const Tensor *tensor) {
  auto srcDes = TensorUtils::get_describe(tensor);
  return srcDes->support_pack16 ? srcDes->channel_pack_num : 4;
}

void TensorUtils::set_tensor_channel_pack(const Tensor *tensor, int pack) {
  auto srcDes = TensorUtils::get_describe(tensor);
  srcDes->channel_pack_num = srcDes->support_pack16 ? pack : 4;
}

void TensorUtils::set_tensor_support_pack(const Tensor *tensor, bool flag) {
  auto srcDes = TensorUtils::get_describe(tensor);
  srcDes->support_pack16 = flag;
}

void TensorUtils::set_tensor_pad(const Tensor *tensor, int left, int right,
                                 int bottom, int top) {
  auto srcDes = TensorUtils::get_describe(tensor);
