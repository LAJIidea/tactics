...
public:
  ...
  virtual MemChunk on_alloc(size_t size, size_t align) {
    return MemChunk(memory_alloc_align(size, MEMORY_ALIGN_DEFAULT), 0);
  }
  virtual void on_release(MemChunk chunk) {
    assert(chunk.second == 0);
    memory_free_align(chunk.first);
  }
};
...
public:
  ...
  virtual MemChunk on_alloc(size_t size, size_t align) override {
    return mParent->alloc(size, false, align);
  }
  virtual void on_release(MemChunk chunk) override { mParent->free(chunk); }

private:
  BufferAllocator *mParent;
};

bool BufferAllocator::compute() { return true; }
std::shared_ptr<BufferAllocator::Allocator>
BufferAllocator::Allocator::create_default() {
  std::shared_ptr<BufferAllocator::Allocator> _res;
  _res.reset(new DefaultAllocator);
  return _res;
}
std::shared_ptr<BufferAllocator::Allocator>
BufferAllocator::Allocator::create_recurse(BufferAllocator *parent) {
  std::shared_ptr<BufferAllocator::Allocator> _res;
...
MemChunk EagerBufferAllocator::alloc(size_t size, bool separate, size_t align) {
  ...
  // alloc otherwise
  auto chunk = mAllocator->on_alloc(size, align);
...
void EagerBufferAllocator::barrier_begin() { assert(mGroups.empty()); }

void EagerBufferAllocator::barrier_end() {
...
DeferBufferAllocator::DeferBufferAllocator(std::shared_ptr<Allocator> parent,
                                           size_t align,
...
size_t DeferBufferAllocator::total_size() const { return mTotalSize; }

void DeferBufferAllocator::barrier_begin() {
  assert(!mBarrrier);
  mBarrrier = true;
}
void DeferBufferAllocator::barrier_end() {
  mBarrrier = false;
  assert(!mBarrrier);
...
void DeferBufferAllocator::begin_group() {
  // do nothing
}
void DeferBufferAllocator::end_group() {
  // do nothing
}
...
bool DeferBufferAllocator::compute() {
  if (mPtr.ptr()) {
    return true;
  }
  mTotalSize = 0;
  ...
  mPtr = mAllocator->on_alloc(mTotalSize, mAlign);
  if (mPtr.ptr() == nullptr) {
    return false;
  }
